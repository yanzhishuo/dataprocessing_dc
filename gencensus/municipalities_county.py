...
#refdeduplication('census_municipalities_gen_0523_2000','ref')
#type 1
Modal = ['', '啊', '哦']
title_pattern = ['', '我', '我的']
pronounce = ['那个', '']
...
def census_type1():
    ...
    return census_raw1, census_address1, census_c, census_county

def census_type2():
    census_address2 = choice(city_county)
    print(census_address2 +'2')
    census_c, census_county = cut_mu(census_address2)
    census_add1 = census_address2
    census_raw2 = Modal[windex([9.5,0.3,0.2])]+title_pattern2[windex([7,3])]+verb2[windex([3,7])]+census_add1+tail[windex([3,7])]
    return census_raw2, census_address2, census_c, census_county

def gen_census():
    index = choice([0,1])
    census_list =[census_type1(), census_type2()]
    census_ref, census_write, census_c, census_county = census_list[index]
    return census_ref, census_write, census_c, census_county
...
if __name__ =="__main__":
    ...
    for i in range(num_data):
        ...
        census_dict['id'] = i
        census_dict['ref'],census_dict['write'], census_dict['city'], census_dict['county']  = gen_census()
    ...
    file = open('/home/yzs/gendata/census_municipalities_county_gen_0523_2000.json', 'w')
    file.write(obj)
    file.close()